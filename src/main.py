...
from rgcn.utils import build_sub_graph, build_dense_graph, neighbors_2_hop_retrieve
from rgcn.utils import mk_dir
...
def test(model, args, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, static_graph, time_list, history_time_nogt, mode):
    ...
    identity_list = [snap for snap in history_list[-args.identity_len:]]
    ...
    for time_idx, test_snap in enumerate(tqdm(test_list)):
        ...
        his_dense_graph = build_dense_graph(num_nodes, num_rels, identity_list, time_idx, use_cuda, args.gpu)
        # node_neighbors_dict = neighbors_2_hop_retrieve(his_dense_graph)
        node_neighbors_dict = None
        ...
        # neighbors_matrix = utils.get_neighbors_matrix(triplets, neighbors, num_entity)
        test_triples, final_score, final_r_score = model.predict(history_glist, his_dense_graph, num_rels, num_nodes, static_graph, test_triples_input, one_hot_tail_seq, one_hot_rel_seq, node_neighbors_dict, use_cuda)

      
        head_entity_list = None
        mrr_filter_snap_r, mrr_snap_r, rank_raw_r, rank_filter_r = utils.get_total_rank_with_tail(test_triples, final_r_score, all_ans_r_list[time_idx], node_neighbors_dict, head_entity_list, eval_bz=1000, rel_predict=1, is_en=False)
        mrr_filter_snap, mrr_snap, rank_raw, rank_filter = utils.get_total_rank_with_tail(test_triples, final_score, all_ans_list[time_idx], node_neighbors_dict, head_entity_list, eval_bz=1000, rel_predict=0, is_en=False)
        ...
        if args.multi_step:
            ...
            identity_list.pop(0)
            identity_list.append(test_snap)
        idx += 1
    
    mrr_raw, hit_result_raw, _ = utils.stat_ranks(ranks_raw, "raw_ent")
    mrr_filter, hit_result_filter, _ = utils.stat_ranks(ranks_filter, "filter_ent")
    mrr_raw_r, hit_result_raw_r, _ = utils.stat_ranks(ranks_raw_r, "raw_rel")
    mrr_filter_r, hit_result_filter_r, _ = utils.stat_ranks(ranks_filter_r, "filter_rel")
...
def run_experiment(args, history_len=None, n_layers=None, dropout=None, n_bases=None, angle=None, history_rate=None):
    ...
    mrr_raw_r = [0,0,0]
    mrr_filter_r = [0,0,0]
    hit_result_raw = [0,0,0]
    hit_result_filter  = [0,0,0]
    hit_result_raw_r = [0,0,0]
    hit_result_filter_r = [0,0,0]
    mrr_filter_repeat = [0,0,0]
    mrr_filter_no_repeat = [0,0,0]
    hit_result_filter_repeat = [0,0,0]
    hit_result_filter_no_repeat = [0,0,0]
    # load graph data
    ...
    model_name = "gl_rate_{}-{}-{}-{}-ly{}-dilate{}-id{}-his{}-weight_{}-discount_{}-angle_{}-dp{}_{}_{}_{}-gpu{}-{}"\
        .format(args.history_rate, args.dataset, args.encoder, args.decoder, args.n_layers, args.dilate_len, args.identity_len,args.train_history_len, args.weight, args.discount, args.angle,
    ...
    # limit the use of training set
    train_use_ratio = args.train_use_ratio
    use_trainset_len = int(len(train_times)*(1-train_use_ratio))

    train_list = train_list[use_trainset_len:]
    train_times = train_times[use_trainset_len:]

    if args.test and os.path.exists(model_state_file):
        mrr_raw, mrr_filter, mrr_raw_r, mrr_filter_r, hit_result_raw, hit_result_filter, hit_result_raw_r, hit_result_filter_r, mrr_filter_repeat, mrr_filter_no_repeat, hit_result_filter_repeat, hit_result_filter_no_repeat = test(model, args,
        ...
        for epoch in range(args.n_epochs):
            ...
            for train_sample_num in tqdm(idx):
                ...
                if train_sample_num - args.identity_len<0:
                    identity_list = train_list[0: train_sample_num]
                else:
                    identity_list = train_list[train_sample_num - args.identity_len: train_sample_num]
                
                # generate history graph
                history_glist = [build_sub_graph(num_nodes, num_rels, snap, use_cuda, args.gpu) for snap in input_list]
                his_dense_graph = build_dense_graph(num_nodes, num_rels, identity_list, train_sample_num, use_cuda, args.gpu)
                # node_neighbors_dict = neighbors_2_hop_retrieve(his_dense_graph)
                node_neighbors_dict = None
                ...
                loss_e, loss_r, loss_static = model.get_loss(history_glist, his_dense_graph, output[0], static_graph, one_hot_tail_seq, one_hot_rel_seq, node_neighbors_dict,num_nodes,use_cuda)
            ...
            if epoch and epoch % args.evaluate_every == 0:
                mrr_raw, mrr_filter, mrr_raw_r, mrr_filter_r, hit_result_raw, hit_result_filter, hit_result_raw_r, hit_result_filter_r= test(model,
                                                                    ...
                                                                    mode="train")
                # model, args, history_list, test_list, num_rels, num_nodes, use_cuda, all_ans_list, all_ans_r_list, model_name, static_graph, time_list, history_time_nogt, mode
            ...
            if count >= args.early_stop:
                print(f"\nEarly stopping, because no enhancement after {args.early_stop} continuous epochs!") 
                break
...
if __name__ == '__main__':
    ...
    parser.add_argument("--early-stop", type=int, default=3,
                        help="number of early stop epochs")
    ...
    # training set utilization ratio
    parser.add_argument("--train-use-ratio", type=float, default=0.1,
                        help="training set utilization ratio")
    ...
    if args.grid_search:
        ...
        for i, grid_entry in enumerate(list(grid)):
            ...
            mrr_raw, mrr_filter, mrr_raw_r, mrr_filter_r, hit_result_raw, hit_result_filter, hit_result_raw_r, hit_result_filter_r, mrr_filter_repeat, mrr_filter_no_repeat, hit_result_filter_repeat, hit_result_filter_no_repeat = run_experiment(args, grid_entry[0], grid_entry[1], grid_entry[2], grid_entry[3], grid_entry[4], grid_entry[5])
            hits = [1, 3, 10]
    ...
    # single run
        ...
        # raw setting
        raw_mrr_list, raw_hits1_list, raw_hits3_list, raw_hits10_list = [],[],[],[]
        filter_mrr_list, filter_hits1_list, filter_hits3_list, filter_hits10_list = [],[],[],[]
        for i in range(1):
            mrr_raw, mrr_filter, hits_raw, hits_filter = run_experiment(args)
            # raw
            raw_mrr_list.append(mrr_raw)
            raw_hits1_list.append(hits_raw[0])
            raw_hits3_list.append(hits_raw[1])
            raw_hits10_list.append(hits_raw[2])
            # time filter
            filter_mrr_list.append(mrr_filter)
            filter_hits1_list.append(hits_filter[0])
            filter_hits3_list.append(hits_filter[1])
            filter_hits10_list.append(hits_filter[2])

        print("########################raw setting########################")
        print("\nMRR_mean(raw): {:.6f}\t".format(np.mean(raw_mrr_list)))
        print("MRR_std(raw): {:.6f}\n".format(np.std(raw_mrr_list)))

        print("Hits@1_mean(raw): {:.6f}\t".format(np.mean(raw_hits1_list)))
        print("Hits@1_std(raw): {:.6f}\n".format(np.std(raw_hits1_list)))

        print("Hits@3_mean(raw): {:.6f}\t".format(np.mean(raw_hits3_list)))
        print("Hits@3_std(raw): {:.6f}\n".format(np.std(raw_hits3_list)))

        print("Hits@10_mean(raw): {:.6f}\t".format(np.mean(raw_hits10_list)))
        print("Hits@10_std(raw): {:.6f}\n".format(np.std(raw_hits10_list)))

        print("########################time filter########################")
        print("\nMRR_mean(filter): {:.6f}\t".format(np.mean(filter_mrr_list)))
        print("MRR_std(filter): {:.6f}\n".format(np.std(filter_mrr_list)))

        print("Hits@1_mean(filter): {:.6f}\t".format(np.mean(filter_hits1_list)))
        print("Hits@1_std(filter): {:.6f}\n".format(np.std(filter_hits1_list)))

        print("Hits@3_mean(filter): {:.6f}\t".format(np.mean(filter_hits3_list)))
        print("Hits@3_std(filter): {:.6f}\n".format(np.std(filter_hits3_list)))

        print("Hits@10_mean(filter): {:.6f}\t".format(np.mean(filter_hits10_list)))
        print("Hits@10_std(filter): {:.6f}\n".format(np.std(filter_hits10_list)))
