...
class RecurrentRGCN(nn.Module):
    def __init__(self, decoder_name, encoder_name, num_ents, num_rels, num_static_rels, num_words, num_times, time_interval, h_dim, opn, history_rate, sequence_len, num_bases=-1, num_basis=-1,
        ...
        self.r_tail = Relation(200, 0)

        self.W1 = nn.Linear(self.h_dim, self.h_dim, bias=False)
        self.W2 = nn.Linear(self.h_dim, self.h_dim, bias=False)
    ...
    def forward(self, g_list, static_graph, his_dense_graph, mask, use_cuda):
        gate_list = []
        degree_list = []
    ...
    def predict(self, test_graph, his_dense_graph, num_rels, num_entity, static_graph, test_triplets, entity_history_vocabulary, rel_history_vocabulary, neighbors, use_cuda):#,entity_history_vocabulary_20):
        ...
        with torch.no_grad():
            ...
            evolve_embs, _, r_emb = self.forward(test_graph, static_graph, his_dense_graph, mask, use_cuda)
            ...
            entity_history_vocabulary = entity_history_vocabulary.cuda()

            
            score_r = self.raw_mode(embedding, r_emb, time_embs, all_triples)
            # score_h_short = self.history_mode(embedding, r_emb, time_embs, all_triples, entity_history_vocabulary_short)
            ...
            # score = (self.history_rate-0.1) * score_h + (1 - self.history_rate -0.1) * score_r  + 0.2 * score_h_short
    ...
    def get_loss(self, glist, his_dense_graph, triples, static_graph, entity_history_vocabulary, rel_history_vocabulary, neighbors, num_entity, use_cuda):
        self.use_cuda = use_cuda
        ...
        evolve_embs, static_emb, r_emb = self.forward(glist, static_graph, his_dense_graph, mask, use_cuda)
        # if evolve_embs == []:
        #     print("lenjj")
    ...
    def raw_mode(self, pre_emb, r_emb, time_embs, all_triples):
        score = self.decoder_ob1.forward(pre_emb, r_emb, time_embs, all_triples).view(-1, self.num_ents)
    ...
    def rel_raw_mode(self, pre_emb, r_emb, time_embs, all_triples):
        score = self.rdecoder_re1.forward(pre_emb, r_emb, time_embs, all_triples).view(-1, 2 * self.num_rels)
        score = F.softmax(score, dim=1)
        return score
